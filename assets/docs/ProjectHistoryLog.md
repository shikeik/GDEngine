...
`1.10.7.1` **构建与流程 (Build & Workflow)**
- [ ] **[New] 资源同步**: 统一 Gradle 资源同步任务 (Sync Task) 与日志输出，确保构建产物完整性
- [ ] **[Adj] 流程标准化**: 规范化项目的创建、导出与更新流程

**编辑器核心 (Editor Core)**
- [ ] **[Fix] 场景刷新**: 修复脚本跳转场景编辑器时数据不刷新问题 (ScreenManager 增加强制初始化重载)
- [ ] **[Adj] 性能优化**: Hierarchy 树改为 30帧/秒 限制刷新，降低空闲开销
...
**UI 视觉 (Visual Polish)**
- [ ] **[Adj] 面板背景**: 给组件标题栏(Header)和属性面板(Body)增加差异化背景色
- [ ] **[Adj] 属性条目**: 给每个属性行增加略深色背景，提升可读性
- [ ] **[Fix] 样式修复**: 修复 CheckBox 样式透明度问题
- [ ] **[Adj] 输入控件**: Float 输入框限制最大宽度并右对齐
- [ ] **[Fix] 文本布局**: `toString` 绘制支持 `\n` 自动计算行高
