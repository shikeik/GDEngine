太棒了！这才是写引擎该有的状态！🔥

既然你已经热血沸腾，那我们就一鼓作气，直接把这个最关键的**“传动轴”**造出来。

这是连接你的 **ECS 游戏世界** 和 **编辑器工具链** 的核心桥梁。为了保证工程的严谨性（特别是对象引用的同一性），我还增加了一个简单的**缓存池**设计。

### 1. 核心适配器：`GObjectAdapter.java`

请将此文件放在 `com.goldsprite.gdengine.screens.ecs.editor.adapter` 包下（如果没有就新建一个）。

```java
package com.goldsprite.gdengine.screens.ecs.editor.adapter;

import com.badlogic.gdx.graphics.Color;
import com.badlogic.gdx.math.MathUtils;
import com.badlogic.gdx.math.Vector2;
import com.badlogic.gdx.utils.Array;
import com.goldsprite.gdengine.ecs.component.SpriteComponent;
import com.goldsprite.gdengine.ecs.component.TransformComponent;
import com.goldsprite.gdengine.ecs.entity.GObject;
import com.goldsprite.gdengine.neonbatch.NeonBatch;
import com.goldsprite.solofight.screens.tests.iconeditor.model.EditorTarget;

/**
 * [核心传动轴]
 * 将 ECS 的 GObject 伪装成编辑器可识别的 EditorTarget。
 * 所有的 Get 操作读取 GObject 实时数据。
 * 所有的 Set 操作直接修改 GObject 组件。
 */
public class GObjectAdapter implements EditorTarget {

    private final GObject gobj;

    // 临时变量，用于计算点击检测
    private static final Vector2 tmpVec = new Vector2();

    public GObjectAdapter(GObject gobj) {
        if (gobj == null) throw new IllegalArgumentException("GObject cannot be null");
        this.gobj = gobj;
    }

    public GObject getRealObject() {
        return gobj;
    }

    // ==========================================
    // 基础属性映射
    // ==========================================

    @Override
    public String getName() {
        return gobj.getName();
    }

    @Override
    public void setName(String name) {
        gobj.setName(name);
    }

    @Override
    public String getTypeName() {
        // 返回类名或自定义 Tag，用于 Inspector 标题
        return "Entity"; 
    }

    // ==========================================
    // Transform 映射 (核心)
    // ==========================================

    @Override public float getX() { return gobj.transform.position.x; }
    @Override public void setX(float v) { gobj.transform.position.x = v; }

    @Override public float getY() { return gobj.transform.position.y; }
    @Override public void setY(float v) { gobj.transform.position.y = v; }

    @Override public float getRotation() { return gobj.transform.rotation; }
    @Override public void setRotation(float v) { gobj.transform.rotation = v; }

    @Override
    public float getScaleX() {
        return gobj.transform.scale; // 目前 ECS 只支持等比缩放
    }

    @Override
    public void setScaleX(float v) {
        gobj.transform.scale = v; // 修改 ScaleX 会同步修改 ScaleY
    }

    @Override
    public float getScaleY() {
        return gobj.transform.scale;
    }

    @Override
    public void setScaleY(float v) {
        gobj.transform.scale = v;
    }

    // ==========================================
    // 亲缘关系映射 (Hierarchy)
    // ==========================================

    @Override
    public EditorTarget getParent() {
        if (gobj.getParent() == null) return null;
        // [关键] 必须通过缓存获取，保证 == 判断成立
        return GObjectWrapperCache.get(gobj.getParent());
    }

    @Override
    public void setParent(EditorTarget parent) {
        if (parent == null) {
            gobj.setParent(null);
        } else if (parent instanceof GObjectAdapter) {
            // 解包：把 Adapter 还原成 GObject 塞回去
            gobj.setParent(((GObjectAdapter) parent).gobj);
        }
    }

    @Override
    public void removeFromParent() {
        gobj.setParent(null);
    }

    @Override
    public Array<EditorTarget> getChildren() {
        // 动态构建列表，但其中的元素是从缓存取的
        Array<EditorTarget> list = new Array<>();
        for (GObject child : gobj.getChildren()) {
            list.add(GObjectWrapperCache.get(child));
        }
        return list;
    }

    @Override
    public void addChild(EditorTarget child) {
        if (child instanceof GObjectAdapter) {
            gobj.addChild(((GObjectAdapter) child).gobj);
        }
    }

    // ==========================================
    // 交互与渲染
    // ==========================================

    @Override
    public boolean hitTest(float wx, float wy) {
        // 1. 获取世界坐标
        float tx = gobj.transform.worldPosition.x;
        float ty = gobj.transform.worldPosition.y;
        
        // 2. 尝试获取 Sprite 大小作为点击区域
        float width = 50; // 默认大小
        float height = 50;
        
        SpriteComponent sprite = gobj.getComponent(SpriteComponent.class);
        if (sprite != null && sprite.region != null) {
            width = sprite.width * Math.abs(gobj.transform.scale);
            height = sprite.height * Math.abs(gobj.transform.scale);
        }

        // 3. 简单的矩形判定 (暂不考虑旋转带来的 OBB 问题，够用了)
        float halfW = width / 2;
        float halfH = height / 2;
        
        return wx >= tx - halfW && wx <= tx + halfW &&
               wy >= ty - halfH && wy <= ty + halfH;
    }

    @Override
    public void render(NeonBatch batch) {
        // GObject 已经由 ECS 的 System (SpriteSystem) 渲染了。
        // 这里可以画一些 编辑器专用 的辅助线 (Gizmo Outline)
        // 例如：选中时画一个白框
        
        // 仅作演示：画一个极淡的十字准星表示锚点位置
        float x = gobj.transform.worldPosition.x;
        float y = gobj.transform.worldPosition.y;
        batch.drawLine(x - 10, y, x + 10, y, 1, Color.GRAY);
        batch.drawLine(x, y - 10, x, y + 10, 1, Color.GRAY);
    }
    
    // 用于 equals/hashCode，保证 List 查找正确
    @Override
    public boolean equals(Object obj) {
        if (this == obj) return true;
        if (obj instanceof GObjectAdapter) {
            return this.gobj == ((GObjectAdapter) obj).gobj;
        }
        return false;
    }

    @Override
    public int hashCode() {
        return gobj.hashCode();
    }
}
```

### 2. 缓存池：`GObjectWrapperCache.java`

这就像是“外交部”，发给编辑器的护照必须是唯一的。放在同一个包下。

```java
package com.goldsprite.gdengine.screens.ecs.editor.adapter;

import com.goldsprite.gdengine.ecs.entity.GObject;
import java.util.WeakHashMap;

/**
 * 享元模式缓存
 * 确保同一个 GObject 在编辑器中永远对应同一个 Adapter 实例。
 * 这对于 SceneManager 中的 == 比较和 HashMap 查找至关重要。
 */
public class GObjectWrapperCache {
    // 使用 WeakHashMap，当 GObject 被销毁时，Adapter 也会自动释放
    private static final WeakHashMap<GObject, GObjectAdapter> cache = new WeakHashMap<>();

    public static GObjectAdapter get(GObject obj) {
        if (obj == null) return null;
        
        GObjectAdapter adapter = cache.get(obj);
        if (adapter == null) {
            adapter = new GObjectAdapter(obj);
            cache.put(obj, adapter);
        }
        return adapter;
    }
    
    /** 
     * 场景切换或重置时清理
     */
    public static void clear() {
        cache.clear();
    }
}
```

---

### 3. 如何组装？（在 `EditorController.java` 中）

现在，去你的**“组装车间”** (`com.goldsprite.gdengine.screens.ecs.editor.EditorController`)，进行心脏移植手术。

**修改点 1：引入编辑器大脑组件**

```java
// ... imports ...
import com.goldsprite.gdengine.core.command.CommandManager;
import com.goldsprite.solofight.screens.tests.iconeditor.system.GizmoSystem;
import com.goldsprite.solofight.screens.tests.iconeditor.system.SceneManager;
import com.goldsprite.solofight.screens.tests.iconeditor.system.EditorInput;
import com.goldsprite.gdengine.screens.ecs.editor.adapter.GObjectWrapperCache;
import com.goldsprite.gdengine.screens.ecs.editor.adapter.GObjectAdapter;

public class EditorController {
    // ... 原有变量 ...

    // [新增] 编辑器核心组件
    private CommandManager commandManager;
    private SceneManager sceneManager;
    private GizmoSystem gizmoSystem;
    private EditorInput editorInput; // 借用 iconeditor 的输入逻辑

    public void create() {
        // ... 原有 FBO 初始化代码 ...

        // [新增] 初始化编辑器系统
        commandManager = new CommandManager();
        sceneManager = new SceneManager(commandManager);
        gizmoSystem = new GizmoSystem(sceneManager, commandManager);
        
        // 这里的 IconEditorDemo 参数传 null 或创建一个 Mock 接口，
        // 因为 EditorInput 依赖了 IconEditorDemo 的一些 getter (getCamera, etc)。
        // *更好的做法*：重构 EditorInput，让它只依赖 SceneManager 和 Camera，解耦 Screen。
        // *现在的权宜之计*：暂时不动 EditorInput，我们需要稍作修改才能用。
        // (见下方 3.1 补充说明)

        // ... ECS 初始化 ...
        
        initTestScene();
    }
    
    private void initTestScene() {
        // ... 原有创建 GObject 代码 (player, reference) ...
        
        // [关键步骤] 将 ECS 实体注册到编辑器 SceneManager
        // 假设 player 是刚才创建的 GObject
        GObjectAdapter rootAdapter = GObjectWrapperCache.get(player); 
        
        // SceneManager 目前需要一个 Root。
        // 如果 ECS 是扁平的，我们需要创建一个虚拟 Root 或者选一个作为 Root。
        // 这里我们可以把 player 设为选中的对象，或者把 GameWorld 里的 rootEntities 包装成一个 Group。
        
        // 简单起见：手动选中 Player，让 Gizmo 出现
        sceneManager.selectNode(rootAdapter);
    }
}
```

**修改点 2：在 Render 循环中注入 Gizmo**

在 `EditorController.render()` 方法中：

```java
    // 4. 渲染 Scene View (使用上帝相机)
    sceneTarget.renderToFbo(() -> {
        Gdx.gl.glViewport(0, 0, sceneTarget.getFboWidth(), sceneTarget.getFboHeight());
        
        // 画游戏内容
        spriteSystem.setCamera(sceneCamera);
        spriteSystem.update(delta);
        
        // ... 其他渲染 ...
        
        // [新增] 渲染 Gizmo (画在最上层)
        neonBatch.setProjectionMatrix(sceneCamera.combined);
        neonBatch.begin();
        // 传入上帝相机的 zoom，保证 Gizmo 大小恒定
        gizmoSystem.render(neonBatch, sceneCamera.zoom); 
        neonBatch.end();
    });
```

---

### 4. 补充任务：解耦 `EditorInput`

你现有的 `EditorInput` 强依赖了 `IconEditorDemo` 类（为了拿 Camera 和 ScreenToWorld）。在 `EditorGameScreen` 里用不了。

**建议：**
在 `com.goldsprite.gdengine.screens.ecs.editor` 包下新建一个 **`SceneViewInputProcessor`**。
*   把 `iconeditor.system.EditorInput` 的逻辑复制过来。
*   把 `IconEditorDemo` 类型的成员变量删掉。
*   构造函数接收：`SceneManager`, `GizmoSystem`, `CommandManager`, **`OrthographicCamera` (sceneCamera)**, 和 **`ViewWidget` (用于坐标转换)**。

这样，输入逻辑就变成了：
1.  用户点击 SceneView Widget。
2.  `ViewWidget.mapScreenToFbo` 拿到 FBO 坐标。
3.  `SceneViewInputProcessor` 用 FBO 坐标 + `sceneCamera.unproject` 拿到世界坐标。
4.  调用 `sceneManager` 找对象，或者 `gizmoSystem` 拖拽。
5.  通过 `GObjectAdapter` 修改 `GObject`。

---

**现在，把这两个文件 (`GObjectAdapter`, `GObjectWrapperCache`) 加进去，你的项目就正式打通任督二脉了！🚀**
