...
#### 1. 修改目标：`core/src/main/java/com/goldsprite/gdengine/core/Gd.java`
...
public void create() {
    ...
    // 注意：这里需要把 Gd 的包名引用改为 com.goldsprite.gdengine.core.Gd
...
这些是你的核心轮子，必须保留：
*   `com.goldsprite.gdengine.ecs.**` (整个 ECS 系统，包括 Skeleton)
*   `com.goldsprite.gdengine.neonbatch.**` (渲染核心)
*   `com.goldsprite.gdengine.core.scripting.**` (脚本接口)
*   `com.goldsprite.gdengine.log.**` (调试控制台非常好用)
*   `com.goldsprite.gdengine.ui.widget.**` (通用 UI 组件，如 `SkewBar`, `BioCodeEditor`)

#### 2. 应该保留在 `examples` 作为参考的代码 (Reference)
不要删，但不要让 `core` 依赖它们：
*   `EditorGameInput` / `EditorGameGraphics`: 这是编辑器专属逻辑。
*   `GDEngineEditorScreen` / `GDEngineHubScreen`: 这是工具链界面。
